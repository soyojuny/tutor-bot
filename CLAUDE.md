...
### 1. 인증 및 세션 관리 (Netflix 스타일 2단계 인증)

- **인증 방식**: '가족' 계정은 Google OAuth를 통해 인증하고, 개별 '프로필'은 선택적으로 PIN 코드를 사용합니다. 이 구조는 단일 계정 내 다중 사용자 경험을 제공합니다.
- **로그인 흐름**:
    1.  **Google 로그인**: 사용자는 먼저 Google 계정으로 로그인하여 Supabase 인증 세션을 확보합니다. 이는 '가족' 단위의 최상위 인증입니다.
    2.  **프로필 선택**: 로그인 후, 해당 가족에 속한 프로필 목록이 표시됩니다. 사용자는 이 중에서 자신의 프로필을 선택합니다.
    3.  **PIN 코드 확인 (선택적)**: 프로필에 PIN이 설정된 경우에만 PIN 입력이 요구됩니다. 부모 프로필 보호 등에 사용될 수 있습니다.
    4.  **프로필 세션 발급**: 인증이 완료되면 `POST /api/auth/login` API는 선택된 **프로필 정보**가 담긴 커스텀 JWT(`tutor-session`)를 생성하고, 이를 `httpOnly` 쿠키에 담아 응답합니다.
- **세션 유지 및 프로필 전환**:
    - 앱의 모든 인가(Authorization)는 이 **프로필 세션 JWT**를 기준으로 이루어집니다.
    - `useAuth` 훅과 `authStore`는 `/api/auth/me` 엔드포인트를 호출하여 현재 선택된 프로필 세션을 확인하고 상태를 관리합니다.
    - 사용자가 프로필을 전환하면, 위 로그인 흐름의 2단계부터 다시 시작하여 새로운 프로필 세션 쿠키를 발급받습니다.
