...
- PointsDisplay (balance, trophy icon, sizes)

**Phase 6: 활동 관리 시스템 (부모)**
- activityStore (Zustand) - CRUD, 상태 관리
- ActivityForm 컴포넌트 (생성/수정 모달)
- manage-activities 페이지 (목록, 필터링, 검증)
- API routes (/api/activities, /api/activities/[id], /api/activities/[id]/verify)
- 완전한 활동 생명주기 (pending → in_progress → completed → verified)
...
**Phase 8: 포인트 시스템**
- pointsStore (Zustand) - 잔액 조회, 거래 내역
- points_ledger 통합 (balance_after 기반 잔액 계산)
- API routes (/api/points)
- 활동 검증 시 자동 포인트 지급
- 보상 교환 시 자동 포인트 차감

**Phase 9: 보상 시스템**
- rewardStore (Zustand) - CRUD, 교환 관리
- RewardForm 컴포넌트 (부모)
- RewardCard 컴포넌트 (아이)
...
- 부모 대시보드 (아이별 포인트, 통계, 빠른 링크, 최근 활동/교환)
- 실시간 데이터 fetching 및 통계 계산
